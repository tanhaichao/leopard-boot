 ...
 *
 * <p>
 * 41位的时间序列（精确到毫秒，41位的长度可以使用69年） 10位的机器标识（10位的长度最多支持部署1024个节点） 12位的Sequence序列号（12位的Sequence序列号支持每个节点每毫秒产生4096个ID序号）
 *
 * <p>
 * 结构如下(每部分用-分开):<br>
...
public class SnowflakeIdWorker {
	...
	public SnowflakeIdWorker(long workerId, long dataCenterId) {
		...
		// logger.info(String.format("worker starting. timestamp left shift %d, datacenter id bits %d, worker id bits %d, sequence bits %d, workerid %d", TIMESTAMP_LEFT_SHIFT_BITS,
		// DATA_CENTER_ID_BITS,ID_BITS, SEQUENCE_BITS, workerId));
	}
