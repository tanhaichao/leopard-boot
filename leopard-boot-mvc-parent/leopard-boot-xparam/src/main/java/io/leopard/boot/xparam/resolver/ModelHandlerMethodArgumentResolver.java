...
import io.leopard.lang.inum.dynamic.DynamicEnumUtil;
...
public class ModelHandlerMethodArgumentResolver extends AbstractNamedValueMethodArgumentResolver implements XParamResolver {
	...
	protected Object resolveName(String name, MethodParameter parameter, NativeWebRequest request) throws Exception {
		...
		for (Field field : clazz.getDeclaredFields()) {// FIXME 未支持Bean继承

			String fieldName = field.getName();
			Class<?> type = field.getType();
			// System.err.println("ModelHandlerMethodArgumentResolver fieldName:" + fieldName + " type:" + type.getName());
			Object obj;
			if (List.class.equals(type)) {
				...
				throw new NotImplementedException("Map类型未实现[" + fieldName + ":" + clazz.getName() + "].");
			}
		...
		if (Snum.class.isAssignableFrom(type)) {
			try {
				...
				if ("".equals(value)) {
					...
					// throw new EnumConstantInvalidException(e.getMessage(), e);
					throw e;
				}
			}
		}
